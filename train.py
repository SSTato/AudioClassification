...
add_arg('num_epoch',        int,    150,                       '训练的轮数')
...
def train(args):
    ...
    for epoch in range(args.num_epoch):
        ...
        epoch1 = str(epoch)
        torch.save(model.state_dict(), os.path.join(args.save_model_dir, epoch1 + 'model.pth'))
        torch.save({'last_epoch': torch.tensor(epoch)}, os.path.join(args.save_model_dir, epoch1 + 'model.state'))
        torch.save(optimizer.state_dict(), os.path.join(args.save_model_dir, epoch1 + 'optimizer.pth'))
